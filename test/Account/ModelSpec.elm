  ...
  describe
    ...
    [ describe
        ...
        [ it "returns the next hash"
            <| expect (nextHash <| Just { next = [ "foo", "bar" ] }) toBe
            <| Just "foo"
    ...
    , describe
        ...
        [ it "returns the last not present hash"
            <| expect (nextHashToDownload setupModel.tweets "bar") toBe
            <| Just "qux"
        , it "returns the same hash if it is not there"
            <| expect (nextHashToDownload setupModel.tweets "uno") toBe
            <| Just "uno"
    ...
    , describe "findItem"
        <| [ it "finds the tweet"
              <| expect (findItem setupModel.tweets <| Just "bar") toBe
